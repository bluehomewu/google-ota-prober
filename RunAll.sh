...
# Define config files to exclude
config_files=$(find . -type f -name "*.yml" ! -name "config.yml" ! -path "./.github/workflows/workflow.yml")
...
if (( ${#changed_devices[@]} > 0 )); then
    ...
    msg=$'🚀 *All devices OTA Updates Detected*\n\n'
